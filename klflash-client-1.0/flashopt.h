 ...
 * @return
 */
int writeFileToFlash(const char*, int r, uint8_t *buffer, char *, unsigned long);

/**
 * @brief 处理/P
 * @return
 */
int  writeFileToMainBlock(const char*, uint8_t *buffer,char*, unsigned long);

/**
 * @brief  处理/B
 * @return
 */
int writeFileToBootBlock(const char*,  int r , uint8_t *buffer, char *, unsigned long);

int writeFileToRomHoles(const char*,  int r , uint8_t *buffer, char *, unsigned long);
/**
 * @brief 处理/N
 * @return
 */
int writeFileToNVROMBlock(const char*, uint8_t *buffer, char *, unsigned long);
...
int writeFileToOA2Block(const char*fileName, uint8_t *buffer, char*, unsigned long);
int writeFileToOA3Block(const char*fileName, uint8_t *buffer, char*, unsigned long);

int writeFileToFlashBySlice(const char *, uint32_t address,
			    uint32_t length, uint8_t * buffer, char *rate,
			    char cycle, unsigned long buffer_phy_addr);

int writeOA2toFlash(const char *, uint32_t address, uint32_t length,
		    uint8_t * buffer, char *rate,
		    unsigned long buffer_phy_addr);
//int writeOA3toFlash(const char *, uint32_t address, uint32_t length, uint8_t * buffer, char *rate);
